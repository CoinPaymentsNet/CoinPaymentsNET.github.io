...
?>
        ...
        <form>
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="action" name="action">
                    ...
                    <option>Create a new callback address</option>
                    <option>List all callback addresses</option>
                    <option>Find a callback address by id</option>
                    <option>Update a callback address</option>
                </select>
                </br>
            ...
            <script>
                ...
                selectElement.addEventListener("change", (event) => {
                    ...
                    if (selectElement.value == 'Create a new callback address'){
                    ...
                    else if (selectElement.value == 'Find a callback address by id'){
                        document.getElementById('idForFinding_block').style.display = "block";
                    }
                    else if ( selectElement.value == 'Update a callback address'){
                        document.getElementById('label_block').style.display = "block";
                        document.getElementById('webhookUrl_block').style.display = "block";
                        document.getElementById('idForFinding_block').style.display = "block";
                    }
    ...
    if (!empty($_GET['action'])) {
        ...
        $apiUrl = 'https://api.coinpayments.net/api/v1/merchant/callbacks';

        switch ($_GET['action']) {

            case 'Create a new callback address':
                $method = 'POST';
                $params = [
                ...
                break;
            case 'Find a callback address by id':
                $method = 'GET';
                $apiUrl .= '/' . $_GET['idForFinding'];
                break;
            case 'List all callback addresses':
                $method = 'GET';
                break;
            case 'Update a callback address':
                $method = 'PUT';
                $params = [
                    "label" => $_GET['label'],
                    "webhook" => array(
